...
-- COMANDO PARA EXIBIR TODAS AS TABELAS DO BANCO

-- COMANDOS PARA EXIBIR O CONTEÚDO (REGISTROS) DE CADA TABELA

-- USO DO COUNT + GROUP BY

-- USO DO COUNT + GROUP BY + HAVING 

-- USO DO DISTINCT

-- REALIZE UMA CONSULTA QUE TRAGA O TOTAL DE POPULAÇÃO DE CADA REGIÃO 
-- QUERO VER O NOME DO PAIS COM A MAIOR POPULAÇÃO | Mostre o maior valor da população e o nome do país
-- QUERO VER O NOME DO PAIS COM A MENOR POPULAÇÃO | Mostre o menor valor da população e o nome do país

-- Liste o nome dos países (name) e o nome das cidades (city.name) que são capitais desses países.

-- Liste o nome das cidades (city.name) e o nome dos países (country.name) que pertencem ao continente 'South America'.

-- Mostre o nome dos países (country.name), o nome das cidades (city.name) e a população das cidades (city.population) para países cuja língua oficial é 'English'.
